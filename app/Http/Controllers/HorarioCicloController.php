...
class HorarioCicloController extends Controller
{
    ...
    /**
     * Muestra la vista de gestión de publicación y reapertura de horarios.
     */
    public function gestionCicloIndex()
    {
        $user = Auth::user();
        ...
        // 1. Determinar roles del usuario autenticado (LÓGICA REAL IMPLEMENTADA)
        ...
        // ✅ Esta consulta es CORRECTA y resuelve la relación indirecta (Periodo -> Grupo -> Carrera)
        ...
        return view('usuarios.admin.admin.horarios.publicacion_reapertura', [
        ...
        try {
            ...
            DB::table('periodo_academico')
                ...
                ->update([
                    ...
                    // ✅ MEJORA: Registrar la fecha y hora exacta de la publicación
                    // aunque el campo no esté explícitamente en el DDL provisto, es buena práctica.
    ...
    // --------------------------------------------------------------------------------
    // Lógica para el CU11: Reabrir Horarios (se mantiene sin cambios funcionales)
    // --------------------------------------------------------------------------------
    public function reabrirHorarios(Request $request)
    {
        ...
        $request->validate([
            'id_periodo' => 'required|integer|exists:periodo_academico,id_periodo',
            'motivo' => 'required|string|min:10', // Motivo para registro en historial
        ]);
