...
@section('title', 'Asistencia Marcada')
...
<div class="asist-shell asist-shell--{{ $mood }}">
    ...
    <p class="asist-sub">
        Hola, <strong>{{ $docenteNombre ?? 'Docente' }}</strong>.
        Tu asistencia para hoy ha sido registrada.
    </p>

    <div class="asist-pill asist-pill--{{ $mood }}">
        {{ $estado ?? 'Asistencia en horario' }}
    </div>
    <p class="asist-detail">
        {{ $detalleTiempo ?? 'Información no disponible' }}
    </p>
    ...
    <div class="asist-grid">
        <div class="asist-card">
            ...
            <div class="asist-card-value">
                {{ $materia ?? 'Materia no disponible' }} — {{ $grupo ?? 'Grupo no disponible' }}
            </div>
        </div>
        <div class="asist-card">
            <div class="asist-card-label">Aula</div>
            <div class="asist-card-value">
                {{ $aula ?? 'Aula no disponible' }}
            </div>
        </div>
        <div class="asist-card">
            <div class="asist-card-label">Fecha de sesión</div>
            <div class="asist-card-value">
                {{ \Illuminate\Support\Carbon::parse($fechaSesion)->format('d/m/Y') }}
            </div>
        </div>
        <div class="asist-card">
            <div class="asist-card-label">Horario</div>
            <div class="asist-card-value">
                Programado: {{ $horaProgramada }} · Marcado: {{ $horaMarcado }}
            </div>
        </div>
        <div class="asist-card">
